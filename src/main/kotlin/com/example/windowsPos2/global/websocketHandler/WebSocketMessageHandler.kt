...
class WebSocketMessageHandler : TextWebSocketHandler () {
    ...
    override fun afterConnectionEstablished(session: WebSocketSession) {
        ...
        val userId = searchUserName(session) ?: return
    ...
    // 웹소켓 세션의 URI를 파싱하여 사용자 token값을 추출하는 메서드
    fun searchUserName(session: WebSocketSession): String? {
        // URL파싱
        val uriComponents = UriComponentsBuilder.fromUriString(session.uri.toString()).build()
        // 쿼리 매개변수에 token값을 추출, 웹소켓에 연결할 때 "accessToken" 파라미터를 붙여서 보내야 searchUserName 메소드가 작동
        return uriComponents.queryParams.getFirst("accessToken")
    }

    // 주문 상태를 브로드캐스트하는 메서드
    fun broadcastOrderUpdate(orderManagementDto: OrderManagementDto) {
        val orderUpdateMessage = convertToJson(orderManagementDto)
        println("잘 될거야 : $orderUpdateMessage")
        for (session in sessionMap.values) {
            try {
                // 세션을 통해 메세지 전송
                println("진짜 잘 보내질거야 : ${session.id}")
    ...
    private fun convertToJson(orderManagementDto: OrderManagementDto): String {
